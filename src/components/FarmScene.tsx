...
function Terrain() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < pos.count; i++) {
      const x = pos.getX(i);
      const y = pos.getY(i);
      const z = Math.sin(x * 0.12) * Math.cos(y * 0.12) * 0.25 + (Math.random() - 0.5) * 0.02;
      pos.setZ(i, z);
    }
...
function Plants({ lat, lon }: { lat: number; lon: number }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        setMoistureFactor(Math.min(1, Math.max(0, p / 10)));
      } catch (err) {
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const sway = Math.sin(t * 2 + i) * 0.08 * (0.5 + moistureFactor * 1.2);
      const scale = 0.4 + moistureFactor * 1.0 + (Math.sin(t * 0.5 + i) + 1) * 0.05;
